...
def get_form_submissions(
        ...
        try:
            ...
            submissions_normalized = response.json().get("results", [])
            print(submissions_normalized)
            submissions_raw = response.json()
            # logger.info(f"Retrieved {len(submissions)} submissions for form {form_uid}")
            
            return {
                "results": submissions_raw,
                "count": response.json().get("count", len(submissions_raw)),
